  ...
  if (!authed) {
    return (
      <main className="min-h-screen flex items-center justify-center px-4">
        <form onSubmit={(e) => { e.preventDefault(); if (pw === PW) setAuthed(true); else setPwErr(true); }}
          className="w-full max-w-sm bg-panel border border-edge rounded-xl p-6">
          <h1 className="text-lg font-semibold mb-1">Invisible Ships</h1>
          <p className="text-sm text-muted mb-4">Journal Browser — protected archive.</p>
          <input autoFocus type="password" value={pw} onChange={(e) => { setPw(e.target.value); setPwErr(false); }}
            placeholder="Password" className="w-full bg-ink border border-edge rounded-md px-3 py-2 text-sm outline-none focus:border-accent" />
          {pwErr && <p className="text-crit text-xs mt-2">Incorrect password — please try again.</p>}
          <button className="mt-4 w-full bg-accent/90 hover:bg-accent text-white rounded-md py-2 text-sm font-medium">Enter</button>
        </form>
      </main>
    );
  }
