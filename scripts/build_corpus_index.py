...
def build_readme(names) -> str:
    """The front door.

    Until 28 August the corpus shipped the README written for the original
    711-file document series: "Total files 711, ~827,000 words", "Meta 3", no
    mention of Crime, Public Health, Government Cloud, Concepts, or the terms.
    It was carried forward untouched by every sync because nothing owned it —
    the same failure as the timeline, the concept axes, the summary layer, the
    terms and the gate prose. Six now.

    A reader who unzips this archive opens README.md first. It was telling them
    the wrong size, the wrong contents, and nothing at all about what they are
    permitted to do with it. Sean, 28 August: "a README ... how you can use the
    data. You just have to cite the source if you're gonna use it."

    START-HERE.md remains the detailed index. This is the page before it.
    """
    ...
    with zipfile.ZipFile(ZIP) as z:
        for n in files:
            if n.endswith((".md", ".csv")):
                words += len(re.findall(r"\b[\w'-]+\b", z.read(n).decode("utf8", "replace")))
    top = {}
    for n in files:
        key = n.split("/")[0] if "/" in n else "(root)"
        top[key] = top.get(key, 0) + 1
    rows = "\n".join(
        f"| `{k}/` | {v} | {SECTION_BLURB.get(k, '')} |"
        for k, v in sorted(top.items(), key=lambda kv: -kv[1]) if k != "(root)"
    )
    return f"""# Invisible Ships — the downloadable archive

The complete research archive behind **invisibleships.com**, as plain files.
Compiled by Sean C. Harris.

**{len(files)} files · roughly {words:,} words · generated {date.today().isoformat()}**

Every file opens with a YAML header and holds one coherent unit of content — a
journal day, one recording's transcript, a reference section, a glossary term, a
concept, a dataset. They are named so that any single file can be handed to an
AI assistant on its own and still make sense.

## How you may use this

**You may share it, and you may quote it. Cite it.**

- **Share it** — this archive may be redistributed in its complete, original form.
- **Quote it** — passages may be quoted for reporting, research, comment,
  teaching or criticism, with attribution to Sean C. Harris and
  invisibleships.com.
- **Check it with an assistant** — this corpus is built to be handed to an AI
  system, whole or file by file, to verify or interrogate the findings. That use
  is intended and permitted.

What is not permitted: presenting this work, or any finding in it, as your own;
republishing it in altered form; commercial use without written permission.

Attribution is the condition on all of it. The full terms, the disclaimer and
the scope of what this archive does and does not establish are in
**`meta/IS_META_terms.md`** — read that before quoting anything.

## Before you read

**`meta/IS_META_entry-and-safety.md`** carries the content warning and the note
on safety. This archive documents material referencing coercion, self-harm and
euthanasia, preserved as documentation of external statements. It is worth
reading first.

**`meta/IS_META_why-invisible-ships.md`** explains the name and the premise.
...
**`START-HERE.md`** — the full index, and how to use this archive with an
assistant.

**`concepts/IS_CON_00_findings.md`** — the headline findings, each linked to the
concept it comes from.

**`concepts/IS_CON_00_not-established.md`** — what this archive does *not*
establish. If you are here to check the work rather than use it, start there.

---

*© {date.today().year} Sean C. Harris. All Rights Reserved. Generated by
scripts/build_corpus_index.py — do not edit by hand.*
"""
