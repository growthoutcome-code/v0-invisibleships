...
export default function Header({
  ...
  const btn = (t: Tab, label: string, extra = "") => (
    <button onClick={() => { onTab(t); setOpen(false); }}
      className={`px-2.5 py-1.5 rounded-md text-sm ${tab === t ? "bg-accent/20 text-accent" : "text-muted hover:text-foreground"} ${extra}`}>
      {label}
    </button>
  );
  return (
    <header className="sticky top-0 z-30 bg-background/85 backdrop-blur">
      <div className="mx-auto max-w-6xl px-4 h-14 flex items-center gap-3">
        ...
        <nav className="hidden lg:flex items-center gap-0.5 mx-auto">
          ...
          <button onClick={onSearch} className="px-2.5 py-1.5 rounded-md text-sm text-muted hover:text-foreground inline-flex items-center gap-1.5">
