    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <p className="text-xs text-muted">
          Sized to be usable: the typical file is about {c.medianWords} words and
          the longest is around {approx(c.largestWords)}. <strong>Do not try to
          upload all {c.files} at once</strong> — open{" "}
        ...
        <div className="flex items-center gap-3 mt-2">
          ...
          <a className="ml-auto" href="/api/corpus?from=export_dialog" download onClick={() => track("export_downloaded")}>
            <Button>Download .zip ({(c.zipBytes / 1e6).toFixed(1)} MB)</Button>
          </a>
        </div>
      </DialogContent>
    </Dialog>
